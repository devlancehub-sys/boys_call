...
class DeviceService {
  DeviceService(this._storage, this._api);
  ...
  void printDetails() {
    ...
    _printFcmToken('startup');
    debugPrint('[Device] login_fields: $loginFields');
    if (_storage.isLoggedIn) {
      _storage.logAuthTokens();
    }
  }

  void _printFcmToken(String source) {
    ...
    if (token != null && token.isNotEmpty) {
      ...
      print('[Device] fcm_token ($source): none (voice calls still work via socket)');
    }
  }

  /// Fast path for login — does not block on slow Google/FCM networks.
  ...
  Future<void> syncWithServer() async {
    ...
    try {
      await _api.put(ApiConstants.deviceSync, data: loginFields);
